...
def set_enterprise_branding_filter_param(request, provider_id):
    ...
    if ENTERPRISE_CUSTOMER_BRANDING_OVERRIDE_DETAILS in request.session and (provider_id or ec_uuid):
        # It can be possible that the session key 'ENTERPRISE_CUSTOMER_BRANDING_OVERRIDE_DETAILS' is pre-populated with
        # 'ec_uuid' but at some point the 'provider_id' is given OR may be it is pre-populated with 'provider_id' but
        # later at the point 'ec_uuid' is given. So before setting it, remove the previous one.
        LOGGER.info("Deleting the session key 'ENTERPRISE_CUSTOMER_BRANDING_OVERRIDE_DETAILS'")
        del request.session[ENTERPRISE_CUSTOMER_BRANDING_OVERRIDE_DETAILS]
